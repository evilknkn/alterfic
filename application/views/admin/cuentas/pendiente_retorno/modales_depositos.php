...
<script type="text/javascript">
    ...
    $("#pay-bill").click(function(){
        ...
        $.ajax({
                ...
                dataType: "json",
                url: '<?php echo base_url("cuentas/pagos/pay_bills")?>',
                data: "fecha_pago=" + $("input:text[name=fecha_pago]").val() + "&comprobante="+ $("#ruta_comprobante").val()  + "&id_depositos=" + type+"&monto_pago="+$("#monto").val() + "&empresa_retorno="+$("#empresa_retorno").val()+"&banco_retorno="+$("#id_banco_option").val(), 
                success: function(data)
                {   
